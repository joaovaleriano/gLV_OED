...
def sort_glv_params_laplace(n, seed, r_max, A_diag_mean, A_diag_std, A_off_diag_std):
    """
    sort_glv_params: sorts parameters that lead to a stable gLV system
    growth rates are sorted uniformly between 0 and r_max
    interspecies interactions are sorted normally around zero with A_off_diag_std standard deviation
    ...
    set_nb_seed(seed)
    r = np.random.uniform(0, r_max, n)
    A = np.random.laplace(scale=A_off_diag_std/2**0.5, size=(n,n))
    A -= np.eye(n)*A + np.diag(np.random.laplace(A_diag_mean, A_diag_std/2**0.5, n))
...
def sort_glv_params_A_x_eq_laplace(n, seed, x_eq_max, A_diag_mean, A_diag_std, A_off_diag_std):
    """
    sort_glv_params: sorts parameters that lead to a stable gLV system
    growth rates are sorted uniformly between 0 and r_max
    interspecies interactions are sorted normally around zero with A_off_diag_std standard deviation
